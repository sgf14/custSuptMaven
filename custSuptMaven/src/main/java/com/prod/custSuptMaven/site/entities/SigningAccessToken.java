...
/* class notes- Chap 28 Oauth2 web service security.  POJO for token (token entity), see pg 847.  
 * This custom built method token security method offers an alternative
 * to the basic OAuth2 InMemoryTokenStore (which couldnt be used in a clustered env) and JdbcTokenStore
 * (which are not able to handle signature- based tokens [ie nonces]). Book argues this approach is more secure
 * than bearer tokens which it discourages because of web service security concerns.  See pg 847 and earlier 834 
 * which discusses Authorization Server approaches
 * 
 */
